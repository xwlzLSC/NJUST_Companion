...
const BOOKMARKLET_CODE = [
  ...
  "function parseExams(){var table=document.querySelector('#dataList')||pickTable(function(t){return t.rows.length*(t.rows[0]?t.rows[0].cells.length:0);});if(!table||!table.rows.length)return [];var headers=Array.from(table.rows[0].cells).map(function(cell){return textOf(cell);});function idx(){for(var i=0;i<arguments.length;i+=1){var key=arguments[i];var found=headers.findIndex(function(item){return item.indexOf(key)!==-1;});if(found!==-1)return found;}return -1;}function extractDateText(value){var raw=String(value||'').trim();if(!raw)return '';var direct=raw.match(/(\\d{4})[-/.年](\\d{1,2})[-/.月](\\d{1,2})/);if(direct){return direct[1]+'-'+String(direct[2]).padStart(2,'0')+'-'+String(direct[3]).padStart(2,'0');}var normalized=raw.replace(/[年月]/g,'-').replace(/日/g,'').replace(/\\//g,'-');var match=normalized.match(/(\\d{4})-(\\d{1,2})-(\\d{1,2})/);return match?match[1]+'-'+String(match[2]).padStart(2,'0')+'-'+String(match[3]).padStart(2,'0'):'';}function stripDatePrefix(value){var raw=String(value||'').trim();if(!raw)return '';return raw.replace(/\\d{4}[年./-]\\d{1,2}[月./-]\\d{1,2}日?/g,' ').replace(/[（(]?\\s*(?:星期|周)[一二三四五六日天]\\s*[)）]?/g,' ').replace(/[，,；;]+/g,' ').replace(/\\s+/g,' ').replace(/^[：:、,\\-]+/,'').trim();}var nameIdx=idx('课程名','课程');var dateIdx=idx('考试日期','日期');var timeIdx=idx('考试时间','时间');var roomIdx=idx('考场','地点','教室');var seatIdx=idx('座位','位号');return Array.from(table.rows).slice(1).map(function(row){var cells=Array.from(row.cells).map(function(cell){return textOf(cell);});if(!cells.length)return null;var rawDate=dateIdx>=0?cells[dateIdx]:'';var rawTime=timeIdx>=0?cells[timeIdx]:'';var date=extractDateText(rawDate)||extractDateText(rawTime);var time=stripDatePrefix(rawTime)||stripDatePrefix(rawDate)||rawTime||rawDate;return {name:nameIdx>=0?cells[nameIdx]:cells[0],date:date,time:time,room:roomIdx>=0?cells[roomIdx]:'',seat:seatIdx>=0?cells[seatIdx]:''};}).filter(function(item){return item&&item.name;});}",
