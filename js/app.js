...
const SUB_PAGES = ['classrooms', 'sites'];
const PAGE_TITLES = {
  home: '南理教务助手',
  schedule: '课表查询',
  grades: '成绩统计',
...
function hasAnyData(data = state.data) {
  return SECTION_KEYS.some(section => {
...
function getMaxWeek() {
  if (!state.data.schedule.length) return 20;
  return state.data.schedule.reduce((max, course) => Math.max(max, course.endWeek || 0), 1);
}
...
function renderServerStatus() {
  ...
  if (state.server.loggedIn) {
    ...
    statusText.textContent = `已登录 ${state.server.username || ''}${nodeText} · 最近自动同步 ${formatRelativeImportTime(state.server.lastSyncAt)}`;
...
async function refreshServerStatus({ silent = false, check = false } = {}) {
  try {
    ...
    if (normalizedRemoteData && (hasAnyData(normalizedRemoteData) || payload.status.lastSyncAt)) {
      state.data = normalizedRemoteData;
...
function getUpcomingCourseInstances(limit = 6, daysAhead = 10) {
  if (!state.data.schedule.length) return [];
  const result = [];
...
function renderHome() {
  ...
  document.getElementById('hero-sub').textContent = hasAnyData()
    ? `${state.server.loggedIn ? '自动同步在线' : '本地缓存模式'} · 最近同步 ${formatRelativeImportTime(state.data.meta.importedAt)}`
...
function renderSchedule(weekday = state.currentWeekday || getTodayWeekday()) {
  ...
  const activeCourses = viewMode === 'week'
    ? state.data.schedule.filter(course => isCourseActiveInWeek(course, selectedWeek))
    : [...state.data.schedule];
  ...
  const boardHeight = totalPeriods * SCHEDULE_SLOT_HEIGHT + (totalPeriods - 1) * SCHEDULE_SLOT_GAP;
  const firstDate = weekDates.find(Boolean);
  const boardClass = `schedule-board ${viewMode === 'semester' ? 'semester-mode' : 'week-mode'}`;

  board.innerHTML = `
    <div class="schedule-board-card ${viewMode === 'semester' ? 'semester-mode' : 'week-mode'}">
      <div class="schedule-board-scroll">
        <div class="${boardClass}" style="--schedule-slot-height:${SCHEDULE_SLOT_HEIGHT}px;--schedule-slot-gap:${SCHEDULE_SLOT_GAP}px;--schedule-board-height:${boardHeight}px;--schedule-slot-count:${totalPeriods};">
            ...
            <div class="schedule-days-grid">
              ${[1, 2, 3, 4, 5, 6, 7].map(day => {
                ...
                const blocks = dayEntries.map((entry, index) => {
                  ...
                  const top = range.startPosition * (SCHEDULE_SLOT_HEIGHT + SCHEDULE_SLOT_GAP);
                  const height = span * SCHEDULE_SLOT_HEIGHT + (span - 1) * SCHEDULE_SLOT_GAP;
                  ...
                  return `
                    <button
                      class="schedule-block ${isLive ? 'live' : ''}"
                      type="button"
  ...
  if (!rooms.length) {
    return `
      <div class="card classroom-result-card">
        ...
        <div class="classroom-result-note">当前条件下没有检索到空闲教室，可能是该楼栋该时段已满，或教务处返回为空。</div>
      </div>
    `;
  }
      ...
      <div class="classroom-room-list">
        ${rooms.map(room => `
          <button
            ...
            type="button"
            onclick="copyClassroomName('${escapeJsString(room.name || room.label)}')"
            title="点击复制教室名"
          >
            <div class="classroom-room-title">${escapeHtml(room.name || room.label)}</div>
            <div class="classroom-room-meta">${escapeHtml(room.capacityText ? `容量/排课：${room.capacityText}` : '当前时段空闲')}</div>
          </button>
        `).join('')}
      </div>
    </div>
      ...
      <div class="control-card">
        <div class="control-row">
          <div>
            ...
            <div class="control-hint">第 ${escapeHtml(String(meta.week))} 周 · ${escapeHtml(meta.weekdayLabel)} · ${escapeHtml(periodGroup.name)}</div>
          </div>
